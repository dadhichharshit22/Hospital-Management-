        ...
        {appointments.map((item, index) => (
          <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b" key={index}>
            ...
            <div className="flex flex-col gap-2 justify-end">
              ...
              {item.qrCodeUrl && (
                <div className="mt-3">
                  <img src={item.qrCodeUrl} alt="Appointment QR Code" className="w-32 h-32 border rounded" />
                  <a href={item.qrCodeUrl} download={`Appointment_QR_${item.slotDate}_${item.slotTime}.png`}>
                    <button className="mt-2 py-2 px-4 bg-blue-500 text-white text-sm rounded">Download QR</button>
                  </a>
                </div>
              )}
            </div>
